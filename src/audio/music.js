...
// ─── 음악 — 후기 낭만 관현악 왈츠 (예비) ────────────────────────
//
// **지금 판을 도는 배경음악은 이쪽이 아니다.** 음악은 파일 한 곡이 루프로
// 깔린다(bgm.js). 이 악보는 그 파일을 못 받아 왔을 때 대신 서는 자리다 —
// 인터페이스가 같아서 부르는 쪽은 둘을 구분하지 않는다(index.js).
//
// 여기에는 오디오 파일이 한 개도 없다. 아래에 있는 것은 **악보**이고,
// Tone.js가 그걸 연주한다. 그래서 이 파일은 코드라기보다 총보에 가깝다 —
// 읽을 때도 그렇게 읽어야 한다.
//
// ══ 왜 3/4인가 ═══════════════════════════════════════════════════
...
export class Music {
  ...
  async build() {
    ...
    // 파일 BGM과 **같은 높이에** 세운다. 이 악보는 혼자 울릴 것을 생각하고
    // 쓴 것이라 musicBus에서 RMS 0.25로 나오는데, 파일 쪽은 0.023이다
    // (둘 다 같은 자리에서 재 봤다 — bgm.js의 LEVEL 주석). 스무 배가 넘는
    // 차이를 그냥 두면, 파일을 못 받아 온 사람만 갑자기 음악에 얻어맞는다.
    // 더킹이 out을 0~1로 오르내리므로 고정 감쇄는 그 뒤에 따로 둔다.
    this.trim = new Tone.Gain(0.12)
    this.reverb.connect(this.out)
    this.out.connect(this.trim)
    this.trim.connect(this.engine.musicBus)
