...
const SAY = {
  ...
  fortDown: 6,
  // 투석기 격파는 요새 격파보다 위다. 요새 하나가 사라지는 것은 순번 하나가
  ...
  beltBlast: 3,
  // 탄이 탄을 친 기폭도 명중이 아니다. 아무것도 안 부순 폭발이지만 빗나간
  // 것도 아니다 — **조준을 다시 한 것**이라 명중(4)과 같은 층에 둔다.
  cueBlast: 4,
  // 혜성 도착 — 굴릴 공이 하나 더 왔다. 판에 몇 안 되는 좋은 소식이라
  // 스윙바이(3)보다 위에 둔다. 대신 판마다 두어 번뿐이라 시끄럽지 않다.
  comet: 4,
  // 순회선 통과 — 혜성과 같은 층이다. 판을 바꾸는 사건은 아니지만
  // **조르그가 아닌 것이 처음 화면에 뜨는 순간**이라 한 줄은 받아야 한다.
  ufo: 4,
  boost: 5,
...
<div class="commsbody">
  ...
  feed(e) {
    ...
    // 탄두가 탄두를 친 기폭도 마찬가지다 — 'nuke'로 흘리면 관제가 "유효타"라고
    // 보고하는데, 부순 것은 아무것도 없고 대신 **날아가던 탄의 진로가 바뀌었다.**
    if (e.kind === 'nuke' && e.cue) return this.raise('cueBlast', SAY.cueBlast)
  ...
  // 요새와 투석기를 **따로** 센다: 둘은 사라졌을 때 하는 말이 다르고
  // (요새는 순번 하나, 투석기는 위협 하나가 통째로), 그 말이 섞이면
  // 관제가 투석기를 "요새"라고 부른다.
  reaped() {
