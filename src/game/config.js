...
export const CFG = {
  ...
  REINF_BASE: 1, REINF_PER_ANTE: 0.5, // 판당 새 요새 수 = BASE + PER_ANTE×안테
  REINF_NEUTRAL: 1,                   // 같이 딸려오는 비위협 조르그 행성 수(상한)
  SYSTEM_CAP: 11,                     // 성계 최대 천체 수 — 넘으면 증원을 줄인다

  // ─── 조르그 레이저 ───────────────────────────────────────────
  // 본성이 지구의 "미래 위치"를 조준해 충전한 뒤 직선으로 쏜다.
  // 충전 시작 시점에 조준선이 고정되므로 ① 다른 행성으로 막거나
  // ② 지구 궤도를 (핵 폭풍으로) 밀어 빗나가게 하는 두 가지 대응이 성립한다.
  LASER_FIRST: 70,                    // 첫 조준까지 인게임 초
  LASER_PERIOD: 95, LASER_JITTER: 25, // 이후 주기 ± 흔들림
  LASER_CHARGE: 9,                    // 충전(조준선 노출) 시간
