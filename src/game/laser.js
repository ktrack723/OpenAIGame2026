...
// ─── 조르그 레이저 ──────────────────────────────────────────────
// 본성이 지구의 **미래 위치**를 계산해 겨눈 뒤 충전하고 쏜다.
//
// 핵심은 "충전 시작 시점에 **방향**(ux,uy)이 고정된다"는 것이다. 계속 추적하면
// 회피가 불가능하고, 발사 순간에 정하면 예고가 무의미하다. 총구는 본성을 따라가되
// 방향은 그대로이므로, 고정해 두면 대응이 넷 생긴다:
//   ① 다른 행성을 조준선 위로 밀어 넣어 막는다 (그 행성이 대신 얻어맞고 밀려난다)
//   ② 핵 폭풍으로 지구를 밀어 조준선에서 비켜난다 (지구는 직격하면 안 되니
//      폭풍의 2차 압력을 쓰는 것 — 기존 blastWave가 그대로 대응 수단이 된다)
//   ③ 본성을 옆으로 민다 — 총구가 밀리면 광선이 통째로 평행이동해 빗나간다
