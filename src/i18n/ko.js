...
export const ko = {
  ...
  'ui.hint': 'TAB 조준/관측 · SHIFT 시간 진행 · S 배속 · ← → 각도 · ↑ ↓ 속도 · [ ] 작약 · F 접촉각 · M 음소거 · 공을 짚으면 제원',
  ...
  'chip.score': '점수', 'chip.shots': '발사', 'chip.earth': '지구',
  'chip.time': '시간', 'chip.bonus': '보너스', 'chip.zoom': '줌',
  ...
  'role.battery.boost': '⇢ 추진기 — 꽂히는 탄을 알아채면 몇 초 뒤 옆으로 비켜선다. 횟수 제한은 없다. 늦게 물린 궤도(가까이서 쏜 탄·막판에 꺾인 탄)만 그대로 맞는다. 태양으로 떨어지는 궤도에 걸리면 로켓을 계속 태워 빠져나온다.',
  ...
  'msg.sunBurn': '{name}이(가) 태양으로 떨어지고 있다 — 로켓을 점화해 궤도를 끌어올린다',
  ...
  'msg.laser.intercept': '레이저 요격 — {yield}Mt 탄두로 광선을 끊었다 (+{score})',
  ...
  'toast.sunBurn': '{name} 태양 탈출 점화',
  'toast.sunBurn.out': '{name} 궤도 회복 — 빠져나갔다',
  'toast.sunBurn.dry': '{name} 연료 소진 — 못 빠져나온다',
  ...
  'toast.timeLeft': '작전 시한 {sec}초 — 서둘러라 (시간 보너스 {bonus})',
  ...
  'vic.row.earth': '지구 체력', 'vic.row.earth.v': '{hp}/{hpMax} — 회복되지 않는다',

  // ── 패배 ──────────────────────────────────────────────────────
  'over.title': '작전 실패', 'over.doom': '그들이 왔다…',
  'over.new': '새 런 시작',
  'over.score': '최종 점수 {score}',
  'over.why': '{why}  ·  최종 점수 {score}',
  ...
  'intro.7.tag': '자, 시작이다',
  'intro.7.line': '조준 모드에서는 시간이 멈춘다. 천천히 재고, 쏘고, 관측해라.',
