...
export const ko = {
  ...
  'lcd.warn.boost': '⚠ 추진기 미사용 — 한 번 비켜선다',
  'lcd.warn.eject': '↩ 벨트까지 갔다 온다 ({v}>{esc})',

  // ── 목표 ──────────────────────────────────────────────────────
  'goal.title': '조르그 요새 섬멸',
  'goal.rule': '조르그 요새(☠)를 전부 없애라. 보통 요새는 체력 1 — 제대로 한 번 처박으면 끝난다. 2판부터 섞여 오는 대형 요새(◇◇)는 체력 2라 두 번 처박아야 한다 — 표식에 체력이 뜬다. 태양·흡수·유폭도 인정된다.',
  ...
  // ── 태그 ──────────────────────────────────────────────────────
  'role.battery': '조르그 요새',
  'role.battery.brief': '☠ 이게 표적이다. **요새는 전부 포대다** — 하나하나가 지구를 겨눠 광선을 충전하고, 시차를 두고 순번대로 쏜다. 하나 부수면 그 순번이 통째로 사라지고, 충전 중에 부수면 그 발은 취소된다. 보통은 체력 1, 대형(2판부터)은 2이고 그만큼 크다. 3판부터는 추진기를 하나씩 달고 오는데, 꽂히는 탄을 알아채고 **태우기까지 몇 초 걸린다** — 늦게 물린 궤도는 그대로 맞는다.',
  ...
  'insp.foot.foe': '체력 1 — 한 번만 제대로 처박으면 끝난다. 태양·특이점·유폭도 인정된다.',
  ...
  'intro.5.hp': '☠ 체력 ◆ 1',
  'intro.6.tag': '걱정 마라',
  'intro.6.line': '성계 둘레는 카이퍼 벨트다. 밖으로 나가는 건 없다 — 튕겨서 돌아온다.',
  'intro.6.boom': '텅!',
