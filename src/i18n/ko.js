...
export const ko = {
  ...
  // ── 지구 관제 ─────────────────────────────────────────────────
  // 관제는 **사람**이고, 사람은 놀라도 무전에서는 짧게 말한다. 그래서 이쪽
  // 목소리의 규칙은 하나다: **본 것을 그대로, 짧게.** 감탄하지 않고, 말을
  // 더듬지 않고, 문장 둘을 넘기지 않는다. 웃기는 자리는 감정이 아니라
  // 사실에서 나온다 — 「얼음만 깼다. 다음 발 준비.」처럼.
  // 저쪽이 태연해서 무섭다면, 이쪽은 무뚝뚝해서 웃긴다.
  'gnd.who': '지구 관제',
  'gnd.fortDown': [
    '표적 소멸. 그 궤도는 이제 비었다.',
    '돌 하나로 요새 하나. 오늘 비용은 그게 전부다.',
    '신호 끊겼다. 마지막까지 뭐라고 떠들던데, 받아 적진 않았다.',
  ],
  'gnd.siegeDown': [
    '초엘리트 정지. 실로가 열린 채로 멎었다. 안은 안 들여다본다.',
    '지구로 굴러올 예정이던 것들, 예정 취소.',
  ],
  'gnd.siegeLock': [
    '경보. 저놈이 겨눈 건 지구가 아니라 공이다. 그 돌부터 치워라.',
    '조준선 나왔다. 광선이 아니다. 저건 당구다.',
    '저놈도 우리랑 같은 걸 배웠다. 공이 밀리면 다음은 우리 집이다.',
  ],
  'gnd.siegeNuke': [
    '조르그 핵 기폭. 저 돌이 이쪽으로 온다. 궤도 계산 붙인다.',
    '밀렸다. 이제 저 돌이 어디로 가느냐만 남았다.',
    '저쪽 탄두, 우리 것과 규격이 같다. 그게 제일 마음에 안 든다.',
  ],
  'gnd.laserHit': [
    '행성 하나 없어졌다. 부서진 게 아니라 없어졌다.',
    '광선 지나간 자리, 체력이고 장갑이고 셀 게 없다.',
  ],
  'gnd.nuke': [
    '기폭 확인. 이제 저 공이 어디로 가느냐가 전부다.',
    '좋은 각이다. 이건 기록해 둬라.',
    '유효타. 궤도가 눈에 보이게 휜다.',
  ],
  'gnd.volatile': [
    '유폭. 반경 안 궤도는 전부 다시 그린다.',
    '연쇄 폭발. 저 구역 계산은 방금 폐기됐다.',
  ],
  'gnd.shatter': [
    '쪼개졌다. 부채꼴 안에 있는 건 전부 맞는다.',
    '행성 하나가 통째로 산탄이 됐다. 한 발이 아니라 일곱 발이다.',
    '파편 확산. 닿는 즉시 사라진다 — 두 번은 없다.',
  ],
  'gnd.boost': [
    '비켰다. 알아챈 발마다 비킨다. 연료는 안 닳는다.',
    '회피 기동. 저쪽이 셈을 다시 시작했다.',
  ...
  'gnd.swing': [
    '스윙바이 성사. 중력이 대신 쳐 줬다.',
    '궤도 꺾였다. 계산대로다.',
    '가속 확인. 이 속도는 공짜다. 우주가 냈다.',
  ],
  'gnd.launch': [
    '탄두 이탈. 여기서부터는 궤도가 한다.',
    '큐 놓았다. 남은 건 보는 일이다.',
    '발사 확인. 되돌릴 방법은 없다. 있었던 적도 없다.',
  ],
  'gnd.laserCharge': [
    '조준선 잡혔다. 저쪽이 우리 집 주소를 알고 있다.',
    '충전 신호. 지금부터 초를 센다.',
    '경보. 광선이 물렸다. 지구를 그 선에서 빼내라.',
  ],
  'gnd.comet': [
  ...
  'gnd.beltBlast': [
    '벨트에 박았다. 판 밖이다. 맞힌 건 없다.',
    '각이 넘쳤다. 성계 밖으로 나갔다.',
    '기폭 확인. 얼음만 깼다. 다음 발 준비.',
  ],
  'gnd.laserMiss': [
    '광선 소진. 지구는 그 자리에 없었다.',
  ...
  // 저기 누가 있고, 그놈에게 지구는 **오늘 처리할 업무 하나**라는 것.
  //
  // 그래서 문장의 규칙이 셋이다. ① 소리치지 않는다 — 목소리를 높이는 쪽은
  // 겁먹은 쪽이다. ② 구체적인 잡무를 말한다: 서류, 재고, 결재, 점심.
  // ③ 예의 바르다. 남의 좋은 수는 그냥 인정하고 기록해 둔다.
  // 저놈이 무서운 이유는 화를 내서가 아니라 **화낼 일도 아니어서**다.
  'zorg.taunt': [
    '지구인. 방금 그 수는 나쁘지 않았다. 적어 뒀다.',
    '서두르지 마라. 오늘 일정은 넉넉하다.',
    '지구는 목록의 삼십칠 번이다. 앞의 서른여섯은 항의가 없었다.',
    '나는 이 궤도에서 십일 년 근무했다. 돌에 맞은 적은 없다.',
    '요새 하나는 재고에서 빼면 된다. 재고는 넉넉하다.',
    '지금은 점심시간이다. 그동안 한 발 더 쏴도 좋다.',
    '이 행성은 서류상 이미 처리됐다. 남은 건 절차다.',
    '조준은 끝났다. 결재를 기다리는 중이다. 위쪽이 느리다.',
    '너희가 돌을 던지는 건 안다. 우리도 예전엔 그렇게 했다.',
    '광선은 직진한다. 그게 전부다. 복잡한 무기가 아니다.',
  ],
  'zorg.death': [
    '이건 보고해야 한다. 서식이 어디—',
    '십일 년이다. 십일 년 동안 한 번도—',
    '재고에서 빼라. 나도 같이 빼—',
    '잠깐. 지금 그 각도는 기록할 가치가—',
    '본대에 전해라. 예정대로였다고. 그렇게만—',
    '아직 휴가가 남았—',
    '좋은 수였다. 인정한다. 다만 절차가—',
    '이럴 줄 알았으면 점심을 먼저—',
  ],
  'zorg.warp': '조르그다. 태양계는 오늘부로 우리 관할이다. 거주민은 그대로 있어도 된다. 오래는 아니다.',
  ...
  'gnd.trailer.fear': '조준선은 지구였다. 저 행성이 끼어들지 않았으면—',
  'gnd.trailer.hit': '행성 하나가 없어졌다. 광선 한 발에.',
  'gnd.trailer.nuke': '핵으로는 안 된다. 계산해 봤다. 한참 모자란다.',
  'gnd.trailer.idea': '그럼 더 큰 걸 던지자. 행성을— 행성에 박자.',
}
